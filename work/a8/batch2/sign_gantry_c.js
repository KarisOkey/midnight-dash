...
export default function (THREE) {
  ...
  for (const sx of [-1, 1]) {
    ...
    for (let i = 0; i < 5; i++) {
      const y = 0.8 + i * 1.22, w = 0.72 - (y / 6.0) * 0.38;
      for (const sz of [-1, 1]) add(B(w * 2, 0.05, 0.05), GALVD, px, y, sz * w);
      for (const ax of [-1, 1]) add(B(0.05, 0.05, w * 2), GALVD, px + ax * w, y, 0);
      if (i < 4) { const a = Math.PI / 2 - Math.atan2(1.22, w * 1.6);
        for (const sz of [-1, 1]) add(B(0.045, w * 2.5, 0.045), GALVD, px, y + 0.61, sz * w, 0, 0, i % 2 ? a : -a);
        for (const ax of [-1, 1]) add(B(0.045, w * 2.5, 0.045), GALVD, px + ax * w, y + 0.61, 0, i % 2 ? -a : a, 0, 0);
      }
    }
  ...
  for (let i = 0; i <= 4; i++) {
    const x = -POSTX + i * (SPAN / 4);
    for (const sz of [-1, 1]) add(B(0.07, TRUSS_H, 0.07), GALVD, x, TRUSS_Y, sz * TRUSS_D / 2);
    if (i < 4) {                                                                   // K bracing: two half-diagonals per bay
      const halfx = SPAN / 8, a = Math.atan2(halfx, TRUSS_H / 2), L = Math.hypot(halfx, TRUSS_H / 2);
      for (const sz of [-1, 1]) {
  ...
  for (let i = 0; i <= 4; i++) { const x = -POSTX + i * (SPAN / 4); for (const sy of [-1, 1]) add(B(0.06, 0.06, TRUSS_D), GALVD, x, TRUSS_Y + sy * TRUSS_H / 2, 0); }
  ...
  for (let i = 0; i < 12; i++) add(B(0.035, 0.06, 0.8), GALVD, -POSTX + 0.32 + i * 0.64, 5.17, WZ);
