...
export default function (THREE) {
  ...
  // 5th argument is an offset ALONG the sweep (world X), not a z position
  const sweepX = (shape, len, mat, y, xoff, rz) => add(ex(shape, len), mat, xoff || 0, y || 0, 0, 0, -Math.PI / 2, rz || 0);
  ...
  for (const sx of [-1, 1]) {
    ...
    add(B(0.22, 0.1, 0.22), RUST, sx * 1.95, RY - 0.16, 0.01);
    for (const sy of [-1, 1]) add(CYL(0.016, 0.016, 0.05, 4), BOLT, sx * 1.95, RY + sy * 0.12, 0.12, Math.PI / 2, 0, 0);
  }
  ...
  for (const px of [-1.6, 0, 1.6]) {
    ...
    for (const ax of [-1, 1]) { add(CYL(0.016, 0.016, 0.06, 4), BOLT, px + ax * 0.12, 0.045, -0.09 + ax * 0.11); add(B(0.05, 0.02, 0.05), RUST, px + ax * 0.12, 0.075, -0.09 + ax * 0.11); }
    add(B(0.2, 0.2, 0.08), GALVD, px, RY, -0.04);
    add(CYL(0.022, 0.022, 0.05, 4), BOLT, px, RY, 0.095, Math.PI / 2, 0, 0);
  }
  // ---- splice plate at the centre post, two reflectors ------------------------------------------------
  add(B(0.22, 0.34, 0.03), GALVD, 0, RY, 0.06);
  for (const sy of [-1, 1]) add(CYL(0.014, 0.014, 0.04, 4), BOLT, sy * 0.07, RY + sy * 0.12, 0.09, Math.PI / 2, 0, 0);
