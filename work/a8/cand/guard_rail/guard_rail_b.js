...
export default function (THREE) {
  ...
  const sweepX = (shape, len, mat, y, z) => add(ex(shape, len), mat, 0, y || 0, z || 0, 0, -Math.PI / 2, 0);
  ...
  for (const sx of [-1, 1]) {
    ...
    for (const sy of [-1, 1]) add(CYL(0.014, 0.014, 0.04, 6), BOLT, sx * (L / 2 - 0.06), RY + sy * 0.11, 0.105, Math.PI / 2, 0, 0);
  }
  ...
  for (const px of [-1.55, 0, 1.55]) {
    ...
    add(ex(poly([[-0.14, -0.14], [0.14, -0.14], [0.14, 0.14], [-0.14, 0.14]],
      [[[-0.1, -0.1], [-0.06, -0.1], [-0.06, -0.06], [-0.1, -0.06]], [[0.06, -0.1], [0.1, -0.1], [0.1, -0.06], [0.06, -0.06]],
       [[-0.1, 0.06], [-0.06, 0.06], [-0.06, 0.1], [-0.1, 0.1]], [[0.06, 0.06], [0.1, 0.06], [0.1, 0.1], [0.06, 0.1]]]), 0.022), GALVD, px, 0.011, -0.08, -Math.PI / 2, 0, 0);
    add(B(0.3, 0.035, 0.3), DARK, px, 0.035, -0.08);
    for (const ax of [-1, 1]) for (const az of [-1, 1]) add(CYL(0.013, 0.013, 0.06, 6), BOLT, px + ax * 0.08, 0.04, -0.08 + az * 0.08);
    add(ex(poly([[-0.08, -0.08], [0.08, -0.08], [0.08, 0.08], [-0.08, 0.08]]), 0.07), GALVD, px, RY, -0.035, 0, 0, 0);
    add(CYL(0.02, 0.02, 0.05, 6), BOLT, px, RY, 0.085, Math.PI / 2, 0, 0);
  }
