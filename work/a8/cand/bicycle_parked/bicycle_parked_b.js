...
export default function (THREE) {
  ...
  const mgF = add(mgGeo, RUST, 0, 0.33, 0.53); mgF.rotation.set(Math.PI / 2, 0, Math.PI / 2); mgF.rotation.x = -Math.PI * 0.5;
  mgF.rotation.set(-Math.PI / 2, 0, 0); mgF.rotation.z = Math.PI / 2; mgF.rotation.order = 'ZYX'; mgF.rotation.set(Math.PI * 0.55, 0, Math.PI / 2, 'ZXY');
  const mgR = add(mgGeo, RUST, 0, 0.33, -0.52); mgR.rotation.set(Math.PI * 0.3, 0, Math.PI / 2, 'ZXY');
  ...
  const gd = add(ex(guard, 0.02), GUARD, 0.06, 0, 0, 0, Math.PI / 2, 0); gd.rotation.set(0, -Math.PI / 2, 0); gd.position.set(0.06, 0, 0);
