...
export default function (THREE) {
  ...
  add(cup, new THREE.CylinderGeometry(0.05, 0.055, 0.014, 8), CUP, 0, 0.007, 0).scale.set(1, 1, 0.7);
  add(cup, new THREE.TorusGeometry(0.052, 0.006, 4, 8), CUPIN, 0, 0.014, 0, Math.PI / 2, 0, 0).scale.set(1, 0.7, 1);
  add(cup, new THREE.CylinderGeometry(0.028, 0.028, 0.006, 8), CUPIN, 0.005, 0.017, 0.005).scale.set(1, 1, 0.7);
  ...
  add(can, new THREE.CylinderGeometry(0.03, 0.03, 0.006, 8), CAN, 0.056, 0.014, 0.012, 0, 0.2, Math.PI / 2).scale.set(1, 1, 0.45);
  add(can, new THREE.CylinderGeometry(0.03, 0.03, 0.006, 8), CAN, -0.056, 0.014, -0.012, 0, 0.2, Math.PI / 2).scale.set(1, 1, 0.45);
  add(can, new THREE.BoxGeometry(0.04, 0.008, 0.03), CAN, 0.03, 0.026, -0.01, 0.1, 0.5, 0.3);              // a fold standing proud
  add(can, new THREE.TorusGeometry(0.007, 0.002, 3, 6), CAN, 0.062, 0.02, 0.012, 0, Math.PI / 2 + 0.2, 0);

  // bag: a low sagging blob, knot drooped to the ground, contents pushing through
  const bag = part('bag', -0.35, 0.35);
  add(bag, new THREE.SphereGeometry(0.09, 8, 5), BAG, 0, 0.05, 0, 0, 0.3, 0).scale.set(1.2, 0.55, 0.9);
  add(bag, new THREE.SphereGeometry(0.05, 6, 4), BAG, 0.06, 0.07, -0.03, 0, 0.3, 0).scale.set(1, 0.7, 0.8);   // a lump in it
  add(bag, new THREE.CapsuleGeometry(0.016, 0.06, 2, 5), BAG, -0.1, 0.02, 0.03, 1.4, 0, 1.1);                    // the neck, on the ground
  add(bag, new THREE.TorusGeometry(0.018, 0.006, 3, 6), KNOT, -0.13, 0.02, 0.045, 0.3, 0.6, 1.1);
  add(bag, new THREE.CapsuleGeometry(0.01, 0.035, 2, 4), BAG, -0.16, 0.01, 0.06, 1.5, 0.4, 0.9);
