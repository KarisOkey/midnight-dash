...
export default function (THREE) {
  ...
  cone(-0.5, -0.10, 0.04, 0.2, false);
  cone(0.5, -0.09, -0.03, -0.4, true);
  ...
  for (let i = 0; i < 5; i++) { const x = -0.44 + i * 0.22, y = x < 0.05 ? 0.44 - (x + 0.58) * 0.0635 : 0.40 + (x - 0.05) * 0.094; const s = new THREE.Mesh(new THREE.CylinderGeometry(0.021, 0.021, 0.09, 8), STRIPE); s.rotation.z = Math.PI / 2 + (x < 0.05 ? 0.063 : -0.094); s.position.set(x, y, -0.11); g.add(s); }
  ...
  for (const x of [-0.5, 0.5]) { const r = new THREE.Mesh(new THREE.TorusGeometry(0.034, 0.008, 4, 8), BLACK); r.position.set(x, x < 0 ? 0.44 : 0.45, -0.11); r.rotation.x = Math.PI / 2; g.add(r); }
  // the run-over cone, on its side and squashed
  const t = cone(0, 0, -Math.PI / 2 + 0.06, 0, false, true);
  t.position.set(0.1, RB * 0.7 - 0.005, 0.18); t.rotation.y = 0.3;
  // torn tin fragment under it
  const tin = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.006, 0.26), TIN); tin.position.set(-0.05, 0.003, 0.17); tin.rotation.y = -0.2; g.add(tin);
  for (let i = 0; i < 4; i++) { const rib = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.006, 0.012), GRIME); rib.position.set(-0.05, 0.007, 0.10 + i * 0.055); rib.rotation.y = -0.2; g.add(rib); }
