...
export default function (THREE) {
  ...
  const RBOX = (w, h, depth, r, mat, x, y, z, bevel = 0.012) => {
    ...
    const geo = new THREE.ExtrudeGeometry(s, { depth: depth - 2 * bevel, bevelEnabled: true, bevelSize: bevel, bevelThickness: bevel, bevelSegments: 2, curveSegments: 4 });
  ...
  MESH(new THREE.CylinderGeometry(0.124, 0.124, 0.03, 18, 1, true), dark, g, bx, 0.015, 0);   // grounding ring
  ...
  const pts3 = arc.getPoints(14).map((p) => new THREE.Vector3(p.x, p.y, 0));
  const handle = MESH(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts3), 14, 0.005, 5, false), steelDark, g, bx, bh - 0.02, 0); handle.rotation.z = -0.35;
  MESH(new THREE.CylinderGeometry(0.012, 0.012, 0.08, 8), timber, g, bx - 0.05, bh - 0.02 + (br + 0.01) * Math.cos(0.35) - 0.01, 0).rotation.z = Math.PI / 2;
