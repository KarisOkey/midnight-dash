...
export default function (THREE) {
  ...
  for (const x of [-0.75, 0.75]) {
    ...
    for (let k = 0; k < 4; k++) { const l = MESH(new THREE.TorusGeometry(0.02, 0.006, 5, 8), steel, g, x, by + bh / 2 + 0.07 + k * 0.03, 0); l.rotation.y = k % 2 ? Math.PI / 2 : 0; }
  }
  // ---- lamps: cans on swing brackets, emissive front discs --------------------------
  g.userData.lights = [];
  for (const s of [-1, 1]) {
    const x = s * (W / 2 - 0.14), y = H + 0.02, z = D / 2 - 0.10;
    CYL(0.015, 0.20, x, H - 0.02, z - 0.04, steelDark, 'y', 6);
    const can = CYL(0.07, 0.16, x, y, z, housing, 'y', 12); can.rotation.x = Math.PI / 2 + 0.5;
    const disc = CYL(0.06, 0.012, x, y - 0.075, z + 0.045, lampMat, 'y', 12); disc.rotation.x = Math.PI / 2 + 0.5;
