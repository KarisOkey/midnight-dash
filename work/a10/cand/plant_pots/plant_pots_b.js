...
export default function (THREE) {
  ...
  const pot = (x, z, r, h, mat) => {
    const p = add(L([[0, 0.01], [0.75, 0.01], [0.78, 0.0], [0.80, 0.08], [0.98, 0.9], [1.05, 0.92], [1.05, 1.0], [0.96, 1.0], [0.93, 0.92]], 10), mat, x, 0, z);
    p.scale.set(r, h, r);
    add(C(r * 0.8, r * 0.8, 0.05, 10, true), GROUND, x, 0.025, z);
    add(C(r * 0.9, r * 0.9, 0.02, 10), SOIL, x, h * 0.92, z);
    add(C(r * 0.9, r * 0.9, h * 0.25, 10, true), RUST, x, h * 0.45, z).scale.set(1, 1, 0.3);  // mineral run patch (flattened ring)
  };
  const blob = (x, y, z, r, mat) => add(L([[0, -0.9], [0.5, -0.8], [0.9, -0.4], [1.0, 0.1], [0.8, 0.6], [0.4, 0.9], [0, 1.0]], 8), mat, x, y, z).scale.set(r, r, r);
  const spray = (x, y0, z, n, len, mat) => { for (let i = 0; i < n; i++) { const a = i * 2.4; add(new THREE.ConeGeometry(0.02, len, 4), mat, x + Math.sin(a) * 0.05, y0 + len / 2 - 0.02, z + Math.cos(a) * 0.05, Math.cos(a) * 0.5, 0, Math.sin(a) * -0.5); } };
  ...
  add(L([[0, 0], [0.12, 0], [0.12, 0.24], [0.10, 0.28], [0.06, 0.30], [0.06, 0.33], [0.05, 0.33]], 12), GALV, 0.50, 0, 0.20);
  ...
  add(L([[0, 0], [0.045, 0], [0.045, 0.015], [0.035, 0.03], [0, 0.03]], 8), GALV2, 0.80, 0.42, 0.10, 0, 0, -0.9);
