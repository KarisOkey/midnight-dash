...
export default function (THREE) {
  ...
  add(C(0.115, 0.115, 0.01, S), RUSTD, 0, 0.04, 0);
  for (let i = 0; i < 12; i++) { const a = i * Math.PI / 6; add(B(0.02, 0.18, 0.012), i % 4 === 1 ? GREEN : i % 4 === 3 ? RED : RUST, Math.sin(a) * 0.12, 0.15, Math.cos(a) * 0.12, 0, a, 0); }
  add(C(0.10, 0.10, 0.50, S, true), STEEL, 0, 0.51, 0);
  add(C(0.104, 0.104, 0.08, S, true), STEEL2, 0.01, 0.42, 0);      // dented band off-centre
  add(C(0.11, 0.10, 0.02, S), STEEL2, 0, 0.27, 0);                 // collar between drum and column
  add(B(0.05, 0.05, 0.006), STEEL2, -0.03, 0.62, 0.10, 0, -0.3, 0);
  add(C(0.06, 0.06, 0.04, S), STEEL2, 0, 0.78, 0);                 // neck
  add(C(0.13, 0.07, 0.10, S, true), STEEL, 0, 0.85, 0);            // bowl
  add(C(0.07, 0.07, 0.01, S), STEEL2, 0, 0.80, 0);
  add(new THREE.TorusGeometry(0.128, 0.008, 4, S), STEEL2, 0, 0.90, 0, Math.PI / 2, 0, 0);
  add(C(0.118, 0.118, 0.012, S), SAND, 0, 0.88, 0);
  for (let i = 0; i < 7; i++) {
    const a = i * 2.2, r = 0.02 + (i % 3) * 0.03;
    add(C(0.004, 0.004, 0.035, 5, true), WHITE, Math.sin(a) * r, 0.89, Math.cos(a) * r, 0, a, Math.PI / 2 - 0.1 * (i % 2));
    add(C(0.004, 0.004, 0.012, 5, true), FILTER, Math.sin(a) * r + Math.cos(a) * 0.022, 0.89, Math.cos(a) * r - Math.sin(a) * 0.022, 0, a, Math.PI / 2);
  }
