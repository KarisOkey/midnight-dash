...
export default function (THREE) {
  ...
  function wheel(x, squash) {
    ...
    const t = MESH(new THREE.TorusGeometry(R - 0.02, 0.022, 6, 16), tyre, w, 0, 0, 0); t.scale.set(1, squash, 1);
    const rim = MESH(new THREE.TorusGeometry(R - 0.045, 0.008, 4, 16), chrome, w, 0, 0, 0); rim.scale.set(1, squash, 1);
    ...
    for (let i = 0; i < 8; i++) {                                                                        // spokes
      ...
      s.rotation.z = i * Math.PI / 8; s.scale.set(1, i % 2 ? squash : 1, 1);
    }
    // mud cake on the tyre, a fat arc
    const mud = MESH(new THREE.TorusGeometry(R - 0.02, 0.026, 5, 8, 1.6), mudTyre, w, 0, 0, 0); mud.rotation.z = -2.2; mud.scale.set(1, squash, 1);
    return w;
  }
  wheel(-0.52, 1.0);
  ...
  const sd = MESH(new THREE.SphereGeometry(0.12, 8, 5), saddle, bike, -0.26, 0.90, 0); sd.scale.set(1.0, 0.28, 0.7);
  for (const s of [-1, 1]) MESH(new THREE.TorusGeometry(0.02, 0.006, 4, 8), chrome, bike, -0.32, 0.86, s * 0.04).rotation.x = Math.PI / 2;
  ...
  const mg1 = MESH(new THREE.TorusGeometry(R + 0.015, 0.012, 4, 12, 2.6), rust, bike, -0.52, R, 0); mg1.rotation.z = 0.3; mg1.scale.set(1, 1, 3.5);
  const mg2 = MESH(new THREE.TorusGeometry(R + 0.015, 0.012, 4, 12, 2.4), rust, bike, 0.52, R, 0); mg2.rotation.z = 0.5; mg2.scale.set(1, 1, 3.5);
