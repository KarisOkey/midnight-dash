...
export default function (THREE) {
  ...
  const unit = (x, y, bodyMat, w) => {
    ...
    cyl(0.20, 0.20, 0.02, 10, [x - w / 2 + 0.30, y, F - 0.005], fanDk, [Math.PI / 2, 0, 0]);
    cyl(0.045, 0.045, 0.03, 6, [x - w / 2 + 0.30, y, F + 0.005], white2, [Math.PI / 2, 0, 0]);
    put(new THREE.TorusGeometry(0.20, 0.012, 3, 10), grille, [x - w / 2 + 0.30, y, F + 0.008]);
    put(new THREE.TorusGeometry(0.11, 0.008, 3, 8), grille, [x - w / 2 + 0.30, y, F + 0.008]);
  ...
  for (const x of [-0.30, 0.70]) { cyl(PR, PR, 0.30, 8, [x, 0.55, PZ], lag); put(new THREE.TorusGeometry(0.07, PR, 4, 6, Math.PI / 2), lag, [x - 0.07, 0.40, PZ], [0, 0, 0]); }
  ...
  put(new THREE.TorusGeometry(0.07, PR, 4, 6, Math.PI / 2), lag, [-1.05, 0.40, PZ], [0, 0, Math.PI / 2]);
  cyl(PR, PR, 1.10, 8, [-1.12, 0.95, PZ], lag);
  put(new THREE.TorusGeometry(0.07, PR, 4, 6, Math.PI / 2), lag, [-1.05, 1.50, PZ], [0, 0, Math.PI]);
  ...
  put(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts.map((q) => new THREE.Vector3(q[0], q[1], q[2]))), 12, 0.05, 6, false), duct);
  cyl(0.06, 0.06, 0.05, 8, [-0.96, 1.03, 0.10], grey);
  cyl(0.06, 0.06, 0.04, 8, [-0.60, 1.20, 0.30], grey, [0, 0, Math.PI / 2]);
  for (let k = 0; k < 5; k++) { const t = 0.1 + k * 0.19; const p = new THREE.CatmullRomCurve3(pts.map((q) => new THREE.Vector3(q[0], q[1], q[2]))).getPoint(t); put(new THREE.TorusGeometry(0.052, 0.008, 3, 6), duct, [p.x, p.y, p.z], [0, 0, 0]).lookAt(new THREE.CatmullRomCurve3(pts.map((q) => new THREE.Vector3(q[0], q[1], q[2]))).getPoint(t + 0.02)); }
  ...
  for (let k = 0; k < 3; k++) put(new THREE.TorusGeometry(0.09, 0.008, 3, 8), cable, [0.84, 0.68 - k * 0.02, 0.10 + k * 0.012], [0.1 * k, 0.15, 0]);
