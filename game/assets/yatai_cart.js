// yatai_cart — WINNER (arm B: profiles). The roof slopes are one scalloped Shape (pantile edge) extruded
...
export default function (THREE) {
  ...
  // on the counter side (+X) the curtain hangs either side of the lightbox, as in the reference
  for (const s of [-1, 1]) for (let i = 0; i < 12; i++) {
    const z = -0.82 + i * 0.13;
    if (s > 0 && z > -0.66 && z < 0.34) continue;
    add(stripGeo, i % 2 ? WHITE : RED, s * 0.6, 1.55, z, 0, Math.PI / 2, 0);
  }
